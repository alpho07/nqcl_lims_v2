...
if (typeof ftable == 'undefined')
{
 ftable = $('#sample_dispatch').dataTable({
		...
		"aoColumns": [
			...
			{"sTitle":"Apportionment", "mData":null,
				"mRender":function(data, type, row){
					if(row.Clients.credit > 0){
						if(row.percentage < 100){
						var billTotal = row.amount; 
							return '<a href = "#" id = "'+row.request_id+'" data-total = "'+billTotal+'" data-cid = "'+row.Clients.id+'" data-inv = "'+row.invoice_no+'" data-cert = "'+row.cert_no+'" data-invoiceamount = "'+row.invociced_amount+'" class = "apportionment" >Apportion</a>'
						}
						else{
							return 'Paid';
						}
...
$('.proforma').live('click', function(e){
	...
	$('.apportionment').live('click', function(e){
	    	e.preventDefault();
	    	
	    	id = $(this).attr("id");
	    	cid = $(this).attr("data-cid");
	    	proforma_amount = $(this).attr("data-total");
	    	invoice_amount = $(this).attr("data-total");

	    	var m_href = '<?php echo base_url()."finance_management/apportionment_view/" ?>' + id + "/" + cid + "/" + proforma_amount + "/" + invoice_amount ;
	   		console.log(m_href);
			    $.fancybox.open({
			        href:m_href,
			        type: 'iframe',
