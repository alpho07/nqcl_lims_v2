...
<html>
    ...
    <body>
	        <a href="<?php echo base_url();?>report_engine/analyst_report_final/12/2016">See Final</a>
            ...
            <div class="content_area" >
                <center>
                    ...
                    <div class="content ">
                        ...
                        <p>Name:<select id='AID'>
						  <?php
						  foreach($analysts as $an):?>
						  <option value="<?php echo $an->id ;?>"><?php echo $an->fname." ".$an->lname ;?></option>
						  <?php endforeach;?>
						</select></p>
                                                <p>Period: 5<sup>th</sup> <select class="mn" name="month" id="MONTH" >
                                <option value="<?php echo date('m') ?>"><?php $currentMonth = date('F');
echo Date('F', strtotime($currentMonth . " last month")); ?></option>
                    ...
                    <div class="footer">
                        ...
                        <p class="btl">Total Number of Samples Received: <input type="text" class="input-lg mn" id="TNS" name="tns"/> </p>
                        <p class="btl">Total Number of Samples Completed: <input type="text" class="input-lg mn" id="TNC" name="tnc"/> </p>
                        ...
                        <table class="tg">
                            ...
                            <tr>
                                <td class="tg-yw4l"><span id="an_name"></span></td>
                                <td class="tg-yw4l"></td>
                                <td class="tg-yw4l"></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td class="tg-yw4l"><input value="<?php echo $name; ?>"/></td>
                                <td class="tg-yw4l">&nbsp;</td>
                                <td class="tg-yw4l">&nbsp;</td>
                            </tr>
                        </table>
...
<script>
    $(document).ready(function () {
        ...
        getCompleted(month, year,$('#AID').val());



        function getCompleted(m, y,a) {
            $.getJSON(base_url + 'getSupervisorAnalystCompleted/' + m + '/' + y+ '/' + a, function (resp) {
                table = $('.RS > tbody');
                $('.RS > tbody').empty();
            ...
            $.getJSON(base_url + 'getSupervisorAnalystPending/' + m + '/' + y+ '/' + a, function (resp) {
                table = $('.PS > tbody');
                $('.PS > tbody').empty();
                $.each(resp, function (i, d) {
        ...
        function loadRS(m,m2, y,a) {
                        $.get(base_url + 'getSupCount/' + m + '/' + m2  + '/' + y  + '/' + a, function (resp) {
         $('#TNS').val(resp); 
        console.log(resp);
                        });

            
            $.getJSON(base_url + 'getSupervisorAnalystCompleted/' + m + '/' + m2  + '/' + y  + '/' + a+'/RS', function (resp) {
                table = $('.RS > tbody');
                $('.RS tbody').empty();
                $.each(resp, function (i, d) {
                    row = '<tr><td class="tg-yw4l Rs_td">1</td><td class="tg-yw4l"><textarea name="nameRS[]">' + d.product_name + '</textarea></td><td class="tg-yw4l"><input type="text" class="labref" name="labrefRS[]" value=' + d.lab_ref_no + '></td><td class="tg-yw4l"><input type="text" class="labref" name="drecRS[]" value=' + d.received + '></td><td class="tg-yw4l"><input type="text" class="labref" name="dretRS[]" ></td><td class="tg-yw4l rem"><a class="reRS" href="#remove">-</a></td></tr>';
                    table.append(row);
                   
                });
                         $('#TNC').val($('.RS tr').length-1);
        ...
        function loadOT(m,m2, y,a) {
            $.getJSON(base_url + 'serveLookup/' + m +  '/' + m2  +'/' + y + '/' + a+'/OT', function (resp) {
                table = $('.OT > tbody');
                $('.OT > tbody').empty();
                $.each(resp, function (i, d) {
        ...
        function loadPS(m,m2, y,a) {
            $.getJSON(base_url + 'getSupervisorAnalystPending/' + m + '/' + m2  + '/' + y +  '/' + a+'/PS', function (resp) {
                table = $('.PS > tbody');
                $('.PS > tbody').empty();
                $.each(resp, function (i, d) {
                    row = '<tr><td class="tg-yw4l Rs_td2">1</td><td class="tg-yw4l"><textarea name="namePS[]">' + d.product_name + '</textarea></td><td class="tg-yw4l"><input type="text" class="labref" name="labrefPS[]" value=' + d.lab_ref_no + '></td><td class="tg-yw4l"><input type="text" class="labref" name="drecPS[]" value=' + d.received + '></td><td class="tg-yw4l"><textarea  class="labref" name="dretPS[]"></textarea></td><td class="tg-yw4l rem"><a class="rePS" href="#remove">-</a></td></tr>';
        ...
        function loadOD(m, y,a) {
            $.getJSON(base_url + 'serveLookupSetails/' + m + '/' + y+ '/' + a, function (resp) {
        ...
        $('#MONTH,#YEAR,#AID,#MONTH2').change(function () {
            m = $('#MONTH').val();
            m2= $('#MONTH2').val();
            y = $('#YEAR').val();
			  a = $('#AID').val();
			  $('#an_name').text( $('#AID option:selected').text())
            loadRS(m,m2, y,a);
            loadOT(m,m2, y,a);
            loadPS(m,m2, y,a);
            loadOD(m,m2, y,a);
