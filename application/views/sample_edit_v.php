...
echo form_open('sample_controller/update',$attributes);
...
<table class="dave-table">
	...
	<tr>
		<td><label>Date Sample Submitted</label></td><td><input type="text" name="submissions_date" value="<?php echo date('d-M-Y', strtotime($mylist[0]['Designation_date'])); ?>" required readonly /></td>
		<input type="hidden" name="submission_date" value="<?php echo $mylist[0]['Designation_date'] ?>" />
	...
	<tr> 
		...
		<td><label>Product Chemical Name</label></td><td><input type="text" name="chemical_name" placeholder="e.g Amoxicillin Trihydrate" value="<?php echo $mylist[0]['active_ing']; ?>" required readonly /></td>
		<td></td>
	</tr>
	
	<tr>
		<td><label>Product Description</label></td><td><textarea name="description" title="Describe how product looks like" required ><?php echo $sampleinfo[0]['Description'] ?></textarea></td>
		<td><label>Product Presentation</label></td><td><textarea type="text" name="presentation" title="Describe how product is presented, Viles, Tablets e.t.c" required><?php echo $sampleinfo[0]['Presentation'] ?></textarea></td>
	</tr>
	...
	<tr>
		...
		<td><label>Product License No</label></td><td><input type="text" name="product_lic_no" placeholder="e.g Raj./ No .1640" value="<?php echo $sampleinfo[0]['Product_lic_no'];?>"  required /></td>
	</tr>
	...
	<tr>
		...
		<td><label>Country of Origin</label></td><td><input type="text" name="country_of_origin" placeholder="e.g India" required value="<?php echo $sampleinfo[0]['Country_of_origin'];?>" id="country_of_origin"/></td>
	</tr>
...
<legend class="misc_title">Reason for Edit</legend>
<table>
<tr><td colspan="5" ><hr /></td></tr>
<tr><td>
<textarea class="chromaconditions" name="edit_notes" required placeholder = "e.g Misspelling in Product Description" ><?php echo $sampleinfo[0]['edit_notes'] ?></textarea>
</td></tr>
</table>
<table>
	<tr>
		<input type ="hidden" name="version_id"  value= "<?php echo $sampleinfo[0]['version_id'] + 1 ?>" />
		<td><input type="submit" value="Update sample information" class="submit-button" name="submit"/></td>
	</tr>
