...
<form name="" action="<?php echo base_url(); ?>'disintegration/save/'<?php echo $labref; ?>" id="disintegrationForm">
    <center>
        <div class="uniformity">
            ...
            <legend><h4>&#8801; NQCL &#187; DISINTEGRATION TEST FORM: TABLETS &#187; SAMPLE : <?php echo $labref; ?> </h4></legend>
                 <hr>
          Run No.:  <select name="repeat" id="i_repeat">
            ...
            <table class="tg-table-light">              
                <tr class="tg-even">
                    ...
                    <td><input type="text" id="dismedium" name="dismedium"  class="dismedium" required value="" tabindex="1"/></td>

                </tr>
                <tr>
                    <td>Duration of Test (min):</td>
                    <td><input type="text" id="dist" name="dist" class="dismedium"  required value="" tabindex="3"/></td>

                </tr>
                <tr class="tg-even">
                    <td>Compendia:</td>
                    <td><textarea type="text" id="compendia" name="compendia"  class="compendia" required placeholder="e.g In-house" cols="50" rows="3"></textarea></td>

                </tr>
				<tr class="tg-even">
                    <td>Results Observed:</td>
                    <td><textarea type="text" id="disro" name="disro"  class="dismedium" required cols="50" rows="3"></textarea></td>
...
<!--                <tr class="tg-even">
            ...
            <p><input type="button" value="Save Disintegration Data" id="Save"/><input type="button" value="Update" id="update"/></p>
        </div>
    </center>
</form>
<script>
    ...
    $('#i_repeat').change(function(){
       ...
       if(i_repeat ==''){
        ...
        $.getJSON("<?php echo site_url('disintegration/load/'.$labref);?>/"+i_repeat,function(data){
         $('#dismedium').val(data[0].dis_medium); 
            $('#dist').val(data[0].duration); 
             $('#disro').val(data[0].results_observed);             
               $('#disco').val(data[0].comments); 
    ...
    $('#update').click(function(){
        data = $('#disintegrationForm').serialize();
        i_repeat=$('#i_repeat').val();
        
        $.ajax({
  type: "POST",
  url: "<?php echo site_url('disintegration/saveDescriptionU/'.$labref);?>/"+i_repeat,
  data: data, 
  dataType:"json",
   success: function(){
    ...
    $(document).ready(function() {
       ...
       /* repeat_no =<?php echo $repeat_no; ?>;
        ...
        $('#Save').click(function() {
            var bad = 0;
            $('#disintegrationForm .dismedium').each(function()
            {
            ...
            if (bad > 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url(); ?>disintegration/save/<?php echo $labref .'/'.$test_id; ?>",
                                        data: dataString2,
                                        success: function() {
																						window.location.href="<?php echo base_url();?>analyst_controller"

                                            $('#Save').prop('value','Save Disintegration Data');
