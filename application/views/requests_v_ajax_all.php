...
//    });
   ...
   // {"sTitle":"Quote","mData":null,
	...
	{"sTitle":"Assign","mData":"id",
		"mRender":function(data, type, row){
			if(row.coa_done_status != 1){
				return '<a class = "assign" id = '+data+' data-rejStatus = '+row.reject_status+' data-reqid ='+row.request_id+' data-client = '+row.client_id+' >Re-Assign</a>';
			}
			else{
				return 'N/A';
			}
	...
	//	"mRender":function(data, type, row){
		...
		//}
	//},
		{"sTitle":"Statement","mData":"reject_reason"},
	...
	"iDisplayLength":10,
	"sAjaxDataProp": "",
	"sAjaxSource": '<?php echo site_url()."request_management/requests_list_all/".$tag;?>',
		});
	}
else {
	rtable.fnDraw();
