...
class Weight_variation extends MY_Controller {
    ...
    public function worksheet() {
        ...
        $data['settings_view'] = "weight_variation_v";
        ...
        $data['settings_view'] = "wv_upload_v"; 
         $data['error'] = "";  
         $this->base_params($data);
    }
    ...
    public function save() {
     ...
     function do_upload() {
        ...
        if (!file_exists("workbooks/" .$labref.'/'.$labref.'.xlsx')) {
            ...
            $objWorksheet = $objPHPExcel->setActiveSheetIndexbyName('weight variation');
           ...
           $B4 = $objWorksheet->getCell('B5')->getValue();
            ...
            $this->db->insert('weight_variation', $post); 
               $this->db->where('test_id', 28);
    ...
    function post_posting() {
        ...
        $posts = array(
            'labref' => $labref,
            'component' => 'weight_variation',
            'component_no' => '0',
            'test_name' => 'weight_variation',
    ...
    function check_repeat_status() {
        ...
        $this->db->where('test_name', 'weight_variation');
    ...
    function save_test() {
        ...
        $final_test_done = array(
            ...
            'test_subject' => 'weight_variation_r',
            'analyst_id' => $analyst_id,
            'priority' => $urgency
        );
    ...
    public function getDisintegrationTestRepeatStatus($labref) {
        $this->db->select_max('repeat_status');
        $this->db->where('labref', $labref);
        $query = $this->db->get('weight_variation');
        return $row = $query->result();
    }

    public function weight_variation_r() {
        ...
        $data['settings_view'] = 'weight_variation_r_v';
        $this->base_params($data);
    }
    ...
    public function base_params($data) {
        $labref=  $this->uri->segment(3);
        $data['title'] = "Weight Variation - ".$labref;
