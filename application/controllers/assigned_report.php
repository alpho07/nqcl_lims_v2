...
class Assigned_report extends MY_Controller {
	...
	function loadScripts(){
            ...
            function getDocumentation($start, $end) {
      
       $title='Documentation Assigned Samples report ('.$start.' to '.$end. ')';
...
';
        ...
        $css.='<p><center><< <a href="'.  base_url().'request_management/assigned_samples'.'">Back</a>    <strong><u>'.$title.'</u></strong></center></p>';
            ...
            $css.='<p></p>';
            $css.='<center>';
          $css.= '<table class="tg"><tr><th class="tg-031e" colspan="6">DOCUMENTATION - ASSIGNED SAMPLES</th></tr>';

            $css.= '<tr>
    <td class="tg-031e he">No:</td><td class="tg-031e he">Labreference No:</td><td class="tg-ugh9 he">Product Name</td><td class="tg-ugh9 he">Analyst</td><td class="tg-ugh9 he">Quantity Issued</td><td class="tg-ugh9 he">Date Issued<br>(d-m-Y)</td></tr>';
            $query1 = $this->db->query("SELECT a_s.*, r.product_name, si.samples_no, p.name FROM `assigned_samples` a_s JOIN request r ON a_s.labref = r.request_id JOIN sample_issuance si on si.lab_ref_no = a_s.labref JOIN packaging p on r.packaging = p.id WHERE a_s.date_time_tracker BETWEEN '$start%' AND '$end%' group by a_s.labref, a_s.analyst_id ")->result();
            $i = 1;
    ...
    <td class="tg-031e">' . $q1->labref . '</td>
      ...
      <td class="tg-031e e12">' . $q1->analyst_name . '</td>
    <td class="tg-ugh9">' . $q1->samples_no . " " . $q1->name . '</td> '
                        . ' <td class="tg-031e e12">' . $ymd . '</td></tr>'
                    . '</tr>';
                $i++;
            endforeach;
