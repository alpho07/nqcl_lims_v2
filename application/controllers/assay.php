...
class Assay extends MY_Controller {
    ...
    function creatWorkbookSheets($labref) {
		//error_reporting(0);
//             $file21 = "Workbooks/" . $labref . "/" . $labref . ".xlsx";0
...
//        $objDrawing->setWorksheet($objPHPExcel->getActiveSheet($sheet));  //save
            ...
            $deter= $this->c_type($labref, $analyst_id);
            if($deter=='tabs'){
                    $objPHPExcel->getActiveSheet()->setCellValue('B57', '=Uniformity!C46') ;
                      $objPHPExcel->getActiveSheet()->setCellValue('A56', 'Each Tablet contains') ;
                    $objPHPExcel->getActiveSheet()->setCellValue('A57', 'Average Tablet Content Weight (mg):') ;
                 }else if($deter=='caps'){
                     $objPHPExcel->getActiveSheet()->setCellValue('B57', '=Uniformity!C46') ;
                 }else{
                    $objPHPExcel->getActiveSheet()->setCellValue('B18', '=Relative!B13') ;
                 }
	 ...
	 $objPHPExcel->setActiveSheetIndex($sheet);
        ...
        $worksheet->getProtection()->setSelectLockedCells(false);
        $worksheet->getProtection()->setSelectUnlockedCells(false);
        $worksheet->getProtection()->setFormatCells(false);
        $worksheet->getProtection()->setFormatColumns(false);
        $worksheet->getProtection()->setFormatRows(true);
        $worksheet->getProtection()->setInsertColumns(true);
        $worksheet->getProtection()->setInsertRows(true);
        $worksheet->getProtection()->setInsertHyperlinks(true);
        $worksheet->getProtection()->setDeleteColumns(true);
        $worksheet->getProtection()->setDeleteRows(true);
        $worksheet->getProtection()->setSort(true);
        $worksheet->getProtection()->setAutoFilter(true);
        $worksheet->getProtection()->setPivotTables(true);
        $worksheet->getProtection()->setObjects(true);
        $worksheet->getProtection()->setScenarios(true);
        ...
        for ($i = 0; $i < count($molecules); $i++) {
            foreach ($sheetnames as $name):
                $worksheet = $objPHPExcel->setActiveSheetIndexByName($name);
                 $F18 = $worksheet->getCell('F18')->getValue();

                if ($name == 'Uniformity' && $F18 =='caps') {
    ...
    function checkforComponent($labref,$heading){
	 ...
	 function checkforAssay($labref,$analyst_id){
        $return= $this->db->where('labref',$labref)
                ->where('analyst_id',$analyst_id)
				->where('test_name','assay')
                ->count_all_results('tests_done');
				if($return > 0){
					return 1;
				}else{
					return 0;
    ...
    ,'{$v['vf4']}','{$v['concetration']}','{$v['repeat_status']}','{$v['analyst_id']}')";
       ...
       // $outputFile = "Workbooks/".$labref."/".$labref.".xlsx";
         ...
         */
                    ...
                    ->setCellValue('B29', $standardsinfo[0]->water_content);
                if($deter=='tabs'){
                    $worksheet->setCellValue('B57', '=Uniformity!C46') ;
                 }else if($deter=='caps'){
                    $worksheet->setCellValue('B57', '=Uniformity!C46') ;
                 }else{
                     $worksheet->setCellValue('B18', '=Relative!B13') ;
                 }
...
$objPHPExcel->getActiveSheet()->setTitle('AD_'.$component1);
    ...
    function che($labref,$activity1='Returning For Supervision'){
        $this->checkExistance($labref, $activity1);
    }
    ...
    function save_testM() {
		...
		$counter =$this->checkforAssay($labref, $analyst_id);
		if($counter < 0){
        $this->db->insert('tests_done', $final_test_done);
	}else{
		echo 'Assay Done';
...
//===================================MULTIPLE ASSAY ====================================//
    ...
    public function approve() {
        ...
        $query = $this->db->delete('supervisor_approvals');
		 $this->approve_data();
