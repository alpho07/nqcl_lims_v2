...
//    }
   ...
   for($i=0;$i<count($cost);$i++){
	...
	function saveInvoice($labref){
		...
		$tables_main = array('invoice');
		$tables_aux = array('invoice_tests');
		
		$tests = $this->input->post("test");
	
		
		foreach($tables_aux as $table){
			
			//Delete current existing 
			$this->db->where('lab_ref_no',$labref)->delete($table);	
		
			//Get columns to be populated
			${$table."_columns"} = $this->getCommentedColumns($table);
			
			$table_instance = ucfirst($table);
		
			foreach(${$table."_columns"} as $k => $v){
					${$v['COLUMN_NAME']} = $this->input->post($v['COLUMN_NAME']);
					echo $v['COLUMN_NAME'];
					for($i=0;$i<count($tests);$i++){
						${$table."_instance"} = new Invoice_tests();
						${$table."_instance"}->$v['COLUMN_NAME'] = ${$v['COLUMN_NAME']}[$i];
						${$table."_instance"}->save();
					}
			}   
		}
	}
